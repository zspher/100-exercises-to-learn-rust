...
enum TicketNewError {
    TitleError { explanation: String },
    DescriptionError { explanation: String },
}
...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    ...
    match ticket {
        Err(TicketNewError::TitleError { explanation }) => panic!("{}", explanation),
        Err(TicketNewError::DescriptionError { explanation: _ }) => Ticket {
            title,
    ...
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::TitleError {
                explanation: "Title cannot be empty".to_string(),
            });
        }
        if title.len() > 50 {
            return Err(TicketNewError::TitleError {
                explanation: "Title cannot be longer than 50 bytes".to_string(),
            });
        }
        if description.is_empty() {
            return Err(TicketNewError::DescriptionError {
                explanation: "Description cannot be empty".to_string(),
            });
        }
        if description.len() > 500 {
            return Err(TicketNewError::DescriptionError {
                explanation: "Description cannot be longer than 500 bytes".to_string(),
            });
        }
